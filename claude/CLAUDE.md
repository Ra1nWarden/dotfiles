...
- For non-trivial tasks, always use plan mode first. Do NOT make any code changes during planning.
  1. Enter plan mode to explore the codebase (read, search, research only — no edits).
  2. Write the plan to a dedicated `plan.md` file in the project root. Include: goals, alternative approaches with trade-offs, recommended approach with rationale, files/modules affected, and implementation steps.
  3. Exit plan mode and let me review `plan.md`. I may edit the file directly with feedback.
