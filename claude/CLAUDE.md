...
- For non-trivial tasks, write and push the plan to the blueprints repo as soon as it's generated, BEFORE presenting it for approval. Do NOT make any code changes during planning.
  ...
  2. Once the plan is fully drafted, write it to `$BLUEPRINTS_DIR/<project>/plan/` following the blueprints convention (see the `blueprints` skill in `~/.claude/skills/blueprints/SKILL.md`) and run commit-on-write immediately.
