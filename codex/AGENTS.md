...
- For non-trivial work, produce a decision-complete plan before implementation and persist the plan as a blueprint before making code changes. If Codex Plan Mode is active, stay read-only and present the plan in chat, then persist it as soon as the user switches out of Plan Mode or explicitly asks to save it.
